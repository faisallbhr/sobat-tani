<x-app-layout>
    <div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-5xl">
        <ul class="flex gap-4">
            <li onclick="openCity(event, 'tabs-1')" class="tablinks text-slate-900 text-slate-400 cursor-pointer px-2 py-1 rounded-md mb-2 bg-white shadow">Sedang dikerjakan</li>
            <li onclick="openCity(event, 'tabs-2')" class="tablinks text-slate-400 cursor-pointer px-2 py-1 rounded-md mb-2">Sudah selesai</li>
        </ul>
                ...
                @foreach ($progress as $acc)
                    <tr class="border-b border-neutral-500 bg-neutral-200">
                        ...
                        <td class="py-4  text-slate-800 whitespace-nowrap text-center">
                            <a href="{{ url('/buruhtani/accept/'.$acc->vacancy->slug) }}" class="bg-red-500 hover:bg-red-600 text-xs px-4 py-2 font-semibold rounded">Kirim laporan
                            </a>
                        </td>
                    </tr>
                @endforeach
            ...
            <tbody>
                @foreach ($done as $acc)
                    <tr class="border-b border-neutral-500 bg-neutral-200">
                        ...
                        <td class="px-6 py-4 text-slate-800">{{ $acc->vacancy->category->name }}</td>
                        <td class="py-4  text-slate-800 whitespace-nowrap text-center">
                            <a href="{{ url('/buruhtani/accept/'.$acc->vacancy->slug) }}" class="bg-blue-500 hover:bg-blue-600 text-xs px-4 py-2 font-semibold rounded">Lihat detail
                            </a>
                        </td>
                    </tr>
                @endforeach
