...
<div class="px-4 sm:px-6 lg:px-8 py-8 w-full max-w-9xl flex justify-center">
    <div class="py-6 max-w-4xl relative w-full">
        ...
        <div class="border border-neutral-500 rounded-md ">
            <div class="p-4">
                <h2 class="mb-3 text-lg font-bold">{{ $acc->title }}</h2>
                <div>{!! $acc->body !!}</div>
            </div>
        </div>
        <div class="flex justify-end gap-2">
            <a href="{{ url('/buruhtani/accept') }}"><button class="mt-4 bg-gray-400 px-4 py-2 text-gray-700 font-medium rounded text-sm hover:bg-gray-500 hover:text-gray-300">Kembali</button></a>
            <form action="{{ url('/detail/'.$acc->slug )}}" method="post">
            @csrf
                @can('buruh tani') 
